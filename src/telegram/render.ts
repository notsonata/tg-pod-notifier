...
function normalizeStatusKey(status: string): string {
  return normalizedOrderStatusKey(status);
}
...
function formatTracking(order: NormalizedOrder): string {
  if (order.trackingLinks.length === 0) {
    ...
    if (status === "delivered") {
      return "Delivered, no tracking number";
    }
    if (["shipped", "on-the-way", "in_transit", "out-for-delivery"].includes(status)) {
      return "In transit, no tracking number";
    }
    return "Pending";
  }
...
export function renderDigest(
  ...
  void settings;
  const visibleOrders = orders.filter((order) => !isHiddenOrderStatus(order.status));
  ...
  for (const provider of providers) {
    const providerOrders = visibleOrders.filter((order) => order.provider === provider);
